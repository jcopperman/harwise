...
export function generateInsomniaCollection(samples: ApiSample[], options: InsomniaOptions = {}): string {
  const resources: any[] = [];
...
function generateHeaders(headers: Record<string, string>, maskHeaders: string[]): any[] {
  const result: any[] = [];
...
function generateBody(body?: string, headers?: Record<string, string>): any {
  if (!body) return {};
